...
import { listQueues } from "./queuesAPI";
...
export const listQueuesThunk = createAsyncThunk("queues/listAll", async () => {
  const allQueues = [];
  try {
    ...
    const numberOfPagesLeft = Math.floor(
      currentQueues.pagination.total / currentQueues.pagination.pageSize
    );
...
export const queuesSlice = createSlice({
  ...
  extraReducers: {
    [listQueuesThunk.fulfilled]: (state, action) => {
      return { ...state, success: true, queues: action.payload };
    },
    [listQueuesThunk.rejected]: (state) => {
